...
# ---------------------------------------------------------------------------
# elements_to_eci — kept for backward compatibility and unit tests
# ---------------------------------------------------------------------------

def elements_to_eci(elements: KeplerianElements):
    """
    Convert Keplerian elements to ECI position and velocity via SGP4.

    Kept so existing unit tests and notebooks that import this function
    directly continue to work.

    Returns
    -------
    (ECIVector, ECIVector)
        Position (metres) and velocity (m/s).
    """
    from hybrid_ntn_optimizer.core.types import OrbitType

    dummy = SatelliteDescriptor(
        sat_id="__tmp__",
        plane_index=0,
        slot_index=0,
        elements=elements,
    ...
    earth_sat = build_earth_satellite(dummy, DEFAULT_EPOCH)
    dt_obj = datetime.strptime(DEFAULT_EPOCH, "%Y-%m-%dT%H:%M:%S").replace(tzinfo=timezone.utc)
    t = _TS.from_datetime(dt_obj)
    geo = earth_sat.at(t)
    pos = geo.position.m
    vel = geo.velocity.m_per_s
    return (
        ECIVector(float(pos[0]), float(pos[1]), float(pos[2])),
        ECIVector(float(vel[0]), float(vel[1]), float(vel[2])),
    )