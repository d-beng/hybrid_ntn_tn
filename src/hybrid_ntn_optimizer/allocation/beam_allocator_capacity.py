...
def allocate_ntn_beams(
    ...
    * Fair share is computed once over ALL eligible users in the hex before
    ...
    for needy_hex in hex_needs:
        ...
        print(f"Evaluating hex {hex_id} with satellite {best_sat.satellite_id} at elevation {elevation_deg:.1f}° and slant range {slant_range_km:.1f} km. Off-axis angles to existing beams: {[f'{a:.1f}°' for a in off_axis_angles]}")
        sinr_ntn_db, c_beam_mbps, spec_eff = calculate_ntn_sinr_capacity(
