...
def _parallel_user_worker(args):
    uid, base_lat, base_lon, res, cfg, boundary_polygon = args
    
    # 1. Re-seed the RNG for this specific core using the unique User ID
    # This prevents all 8 cores from generating the exact same mobility vectors!
    np.random.seed(uid)
    random.seed(uid)
    ...
    # 4. Build and return the user
    return _build_user_profile(uid, lat, lon, res, cfg, boundary_polygon)
            ...
            if cfg.population.get("worldpop_tif_restricted", None):        
                ...
                with concurrent.futures.ProcessPoolExecutor(max_workers=num_cores) as executor:
                    ...
                    for spawned_user in executor.map(_parallel_user_worker, tasks, chunksize=chunk_size):
                        ...
                        # Print a progress update every 100,000 users so you know it hasn't crashed
                        if len(users) % 100000 == 0:
                            print(f"   ... spawned {len(users):,} / {total_expected_users:,} users")
                            
        print("✅ WorldPop generation complete!")
